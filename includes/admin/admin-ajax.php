...
function csv_scheduler_test_handler() {
    ...
    try {
        ...
        if ( ! class_exists( 'CSV_Import_Scheduler' ) ) {
            wp_send_json_error( [
                'message' => 'Scheduler-Klasse nicht verfügbar.',
                'debug' => [
                    'class_exists' => false,
        ...
        // Prüfen ob test_scheduler Methode existiert
        if ( ! method_exists( 'CSV_Import_Scheduler', 'test_scheduler' ) ) {
            wp_send_json_error( [
                'message' => 'Scheduler-Test-Methode nicht verfügbar.',
                'debug' => [
                    'class_exists' => true,
                    'method_exists' => false,
                    'available_methods' => get_class_methods( 'CSV_Import_Scheduler' )
                ]
            ]);
        }
        ...
        // Exception-Handling
        $error_details = [
            'message' => 'Scheduler-Test-Exception: ' . $e->getMessage(),
            'debug' => [
